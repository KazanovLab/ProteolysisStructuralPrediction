    ...
    for structure in structure_list:        
        ...
        for feature_file in feature_files:
            ...
            if 'AF-' in structure_name:
                feature_df = feature_df.astype({"structure":str, "chain":str, "AA":str, "num_AA":str, "init_SS_type":str, "SS_type":str, "bp1":int, "bp2":int, "ACC":float, "confidence_score":float, "len_loop":int, "is_terminus":int})
            else:
            ...
            if 'AF-' in structure_name:
                normalise(feature_df, "confidence_score", "confidence_score_N_chain")
                if np.unique(feature_df["confidence_score_N_chain"].isna()) == [True]:
                    warnings.append(f"{structure_name} - uncorrected normalisation of AlphaFold confidence score because of values are same!\n******** WARNING ********\nPredictions may be uncorrected!")
                    feature_df["confidence_score_N_chain"].fillna(0, inplace=True)
                    #continue
            else:
